...
void traverse_and_fill_table(struct ASTNode * root) {
    ...
    if (root->has_curly_braces) {
        ...
        if (root->node_type == NODETYPE_STATEMENTLIST || root->node_type == NODETYPE_STATEMENT ) {
            root->symbolic_register = root->parent->symbolic_register;
        } else {
            root->symbolic_register = new symbolic_register();
            root->symbolic_register->symbolic_register_counter = 0;
        }
        if (root->parent != NULL) {
        ...
        if (root->node_type == NODETYPE_MAINMETHOD) {
            ...
            root->symbolic_register->symbolic_register_counter = 1;
            entry->id = "args";
    ...
    if (root->node_type == NODETYPE_VARDECL) {
        ...
        if (find_method(id) != NULL) {
            ...
            struct SymbolTable * table = root->symbol_table;
            root->symbolic_register->symbolic_register_counter = method_symbol_table[num_entries]->number_of_parameters;
            num_entries++;
        }
